...
Item {
    ...
    property var enabledProviders: {
        ...
        if (deepseekProvider.enabled)
            result.push(deepseekProvider);
        return result;
    }

    property int activeIndex: 0
    property string barFixedProviderId: pluginSettings?.barFixedProviderId ?? ""

    property var activeProvider: {
        if (enabledProviders.length === 0)
            return null;
        if (barDisplayMode === "fixed") {
            for (const p of providers) {
                if (p.enabled && p.providerId === barFixedProviderId)
                    return p;
            }
        }
